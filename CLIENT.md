...
The UNN Room client is the doorway into your own personal node inside the Underground Network. Instead of connecting directly with an entry of the network via SSH, start the client and connect through the client. The client offers a modified SSH experience that extends your SSH session with additional functionality. Beneath the surface it opens a second, hidden channel — a private line between you and the server that turns your SSH session into a control console (via a subtile status bar at the bottom of the terminal). In the window you can see the normal SSH session and visit the network as you would normally.
