...
The network is entered through public entry points—server addresses that act as gateways into the system. Entry points do not have rooms of their own; they simply route visitors into the distributed network of active user nodes. Once inside, visitors can explore the topology, discover active nodes, and interact with the services those nodes expose.
...
We are using a secret port 44322 for SSH and a subprotocol (named unn), as this provides a clean separation of concerns. The main SSH session becomes the user interface and the unn subsystem provides the room functionality.